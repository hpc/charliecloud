...
@test "${tag}: ARG change causes miss" {  # issue #1638
    ch-image build-cache --reset

    # Pull both base images (bug does not manifest unless base is already in
    # cache).
    ch-image pull alpine:3.17
    ch-image pull alpine:3.16

    echo
    echo '*** Build once: all misses'
    run ch-image build -t tmpimg -f ./bucache/argfrom.df ./bucache
    echo "$output"
    [[ $status -eq 0 ]]
    [[ $output = *"1  ARG base='alpine:3.17'"* ]]
    [[ $output = *'2* FROM alpine:3.17'* ]]
    [[ $output = *'3. RUN true'* ]]

    echo
    echo '*** Build again: all hits'
    run ch-image build -t tmpimg -f ./bucache/argfrom.df ./bucache
    echo "$output"
    [[ $status -eq 0 ]]
    [[ $output = *"1  ARG base='alpine:3.17'"* ]]
    ...
    echo
    echo '*** Build a 3rd time with a different base image: should now miss'
    run ch-image build --build-arg base=alpine:3.16 \
                       -t tmpimg -f ./bucache/argfrom.df ./bucache
    echo "$output"
    [[ $status -eq 0 ]]
    [[ $output = *"1  ARG base='alpine:3.16'"* ]]
    [[ $output = *'2* FROM alpine:3.16'* ]]
    [[ $output = *'3. RUN true'* ]]
}
