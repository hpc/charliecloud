...
Common options placed before or after the sub-command:
  ...
  :code:`-q, --quiet`
    Suppress logging info, can be repeated. At level one, suppress logging
    generated by Charliecloud, except for warnings. At level two, also discard
    stderr generated by subprocesses of `ch-image`. At level three, suppress
    all logging, even warnings, and discard all output to stderr.
