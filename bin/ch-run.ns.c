...
#define CHROOT_PUT_OLD "/mnt/host"
...
#ifdef NO_DROP_PRIVS
  #warning "SECURITY: NO_DROP_PRIVS defined: Will run user programs as root"
#endif
...
void enter_udss(const char * image_path)
{
   ...
   if (syscall(SYS_pivot_root, CHROOT_TARGET, CHROOT_TARGET CHROOT_PUT_OLD))
      efatal("pivot_root");
   if (chdir("/")) efatal("chdir(\"/\")");
   if (umount2(CHROOT_PUT_OLD, MNT_DETACH)) efatal("umount2(CHROOT_PUT_OLD)");
...
*/
   ...
   if (ruid != 0 || rgid != 0)
      fatal("must run directly as root in NO_DROP_PRIVS mode (not setuid)");
