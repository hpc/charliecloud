...
@test "${ch_tag}/start" {
    ...
    # start the master
    ch-run -b "${spark_config}:/mnt/0" "$ch_img" -- /opt/spark/sbin/start-master.sh
    sleep 7
    ...
    # start the workers
    # shellcheck disable=SC2086
    $pernode ch-run -b "${spark_config}:/mnt/0" "$ch_img" -- \
...
@test "${ch_tag}/pi" {
    run ch-run -b "${spark_config}:/mnt/0" "$ch_img" -- \
...
@test "${ch_tag}/stop" {
    $pernode ch-run -b "${spark_config}:/mnt/0" "$ch_img" -- /opt/spark/sbin/stop-slave.sh
    ch-run -b "${spark_config}:/mnt/0" "$ch_img" -- /opt/spark/sbin/stop-master.sh
    sleep 2
